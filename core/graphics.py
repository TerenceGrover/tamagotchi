...
class Graphics:
    ...
    def draw_sprite_at(self, x, y, sprite_path, sprite_width=10, sprite_height=10, opacity=1.0):
        """
        ...
        # If opacity is less than 1.0, blend with a black image.
        if opacity < 1.0:
            # Create a black image of the same size
            black_img = Image.new("RGBA", img.size, (0, 0, 0, 255))
            img = Image.blend(black_img, img, opacity)
    ...
    def draw_housing_screen(self, housing_state, stats):
        ...
        player_money = stats.stats["money"]

        # Check if this house is locked
        locked = player_money < current_threshold
        ...
        if housing_state["current_home"]:
            ...
            self.draw_sprite_at(self.matrix_width // 4 - 5, self.matrix_height // 4, current_house["sprite"], sprite_width=48, sprite_height=24)
            ...
            # Draw the house sprite normally if unlocked, else draw a greyscale/locked version
            if locked:
                # Draw the sprite with lower opacity (e.g., 0.5)
                self.draw_sprite_at(self.matrix_width // 4 - 5, self.matrix_height // 4,
                                    current_house["sprite"], sprite_width=48, sprite_height=24, opacity=0.3)
                # Overlay "LOCK" text centered on the screen.
                self.draw_sprite_at(self.matrix_width // 2, self.matrix_height // 2,
                    "assets/sprites/housing/lock.png", sprite_width=10, sprite_height=10)
            else:
                self.draw_sprite_at(self.matrix_width // 4 - 5, self.matrix_height // 4, current_house["sprite"], sprite_width=48, sprite_height=24)
    ...
    def draw_death_social(self, elapsed, sprite_path):
        for i in range(2):
            ...
            self.draw_sprite_at(10 + i * 15 + x_offset, 10, f"assets/sprites/otherTama/tama{i+1}.png")
        if elapsed > 5:
            self.clear_screen()
